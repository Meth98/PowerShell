	...
	foreach ($row in $file_content) {
		...
		if ($match.Count -eq 2) {
			...
			if ($system.Length -eq 0 -or $ip_address.Length -eq 0) {
				...
				"`nTesting the connection with the server '" + $system + " - $ip_address' ..." >> $log
...
function copy_csv_to_remote_host {
	...
	if ($remote_host.Length -ne 0) {
		if ($remote_path.Length -ne 0) {
			...
			if (Test-Connection -Cn $remote_host -Quiet -Count 2) {
				...
				if (! (Test-Path $share -PathType Container)) {
					...
					try {
						...
						Write-Host -ForegroundColor "green" "The file has been copied on $share!"
					} catch {
						Write-Host -ForegroundColor "red" "$remote_host - Failed to copy the CSV on $share!"
			...
			Write-Host -ForegroundColor "yellow" "Attention! I don't understand where I should copy the csv file!"
		}
	}
}
